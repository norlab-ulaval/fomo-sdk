...
class MTLParser(object):
    def __init__(self, mtlFilePath, baseDir, use_sha=False):
        self.materials = {}
    ...
    def reduceMaterialDefinitions(self):
        for index, name in enumerate(self.materials):
            if self.use_sha:
                code = materialToSHA(self.materials[name])
            else:
                code = f"part{index:04d}"
            self.reducedMaterialMap[name] = code
            self.reverseReduceMap[code] = name

        print(
            "Reducing materials from %s to %s"
            % (len(self.reducedMaterialMap), len(self.reverseReduceMap))
        )

    def applyMaterialToRepresentation(self, name, representation):
        print("Apply material to representation")
        ...
        if "map_Kd" in material:
            if name not in self.textures:
                ...
                texture = CreateTexture(
                    os.path.join(self.baseDir, material["map_Kd"][0])
                )
        ...
        if "Ka" in material:
            ...
            self.representationsParameters[name]["AmbientColor"] = [
                float(n) for n in material["Ka"]
            ]

        if "Ks" in material:
            representation.SpecularColor = [float(v) for v in material["Ks"]]
            self.representationsParameters[name]["SpecularColor"] = [
                float(v) for v in material["Ks"]
            ]

        if "Kd" in material:
            representation.DiffuseColor = [float(v) for v in material["Kd"]]
            self.representationsParameters[name]["DiffuseColor"] = [
                float(v) for v in material["Kd"]
            ]
        ...
        if "Ns" in material:
            ...
            self.representationsParameters[name]["SpecularPower"] = float(
                material["Ns"][0]
            )
        ...
        if "illum" in material:
            ...
            self.representationsParameters[name]["Ambient"] = (
                1.0 if 0 <= float(material["illum"][0]) else 0.0
            )
            self.representationsParameters[name]["Diffuse"] = (
                1.0 if 1 <= float(material["illum"][0]) else 0.0
            )
            self.representationsParameters[name]["Specular"] = (
                1.0 if 2 <= float(material["illum"][0]) else 0.0
            )
        # else:
...
def writeMeshes(meshBaseDirectory, objReader, mtlReader, nameMapping={}):
    nameToFilePath = {}
    ...
    size = len(dsList)
    for name in dsList:
        if name in mtlReader.reverseReduceMap:
            material = mtlReader.materials[mtlReader.reverseReduceMap[name]]
            if "map_Kd" in material:
                name2 = material["map_Kd"][0]

                fullPath = os.path.join(meshBaseDirectory, name2.split(".")[0] + ext)
                ...
                writer.Modified()
                print(
                    f"{(idx + 1)} - {len(dsList[name])}/{size} - {name} => {fullPath}"
                )
                writer.Write()
...
def loadScene(objFilePath: pathlib.Path, objBaseDirectory: pathlib.Path):
    mtlFilePath = objBaseDirectory / f"{objFilePath.stem}.mtl"
    meshBaseDirectory = os.path.join(os.path.dirname(objFilePath), "meshes")

    print(
        f"Object: {objFilePath},\nParent: {objBaseDirectory},\nMaterial: {mtlFilePath},\nMesh: {meshBaseDirectory}"
    )
    mtlReader = MTLParser(mtlFilePath, objBaseDirectory)
    ...
    meshMapping = writeMeshes(meshBaseDirectory, objReader, mtlReader, mapToSha)
    for name in meshMapping:
        print(meshMapping[name], name)
    ...
    with open(
        meshBaseDirectory + "/representations.json", "w", encoding="utf-8"
    ) as text_file:
        text_file.write(
            json.dumps(mtlReader.representationsParameters, indent=2, sort_keys=True)
        )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        # create layout and a view
        layout = simple.CreateLayout(name="MyLayout")
        view = simple.CreateView("RenderView")
        simple.AssignViewToLayout(view, layout)

        objPath = sys.argv[1]
        # Get the absolute path to the directory containing objFilePath
        objAbsPath = pathlib.Path(objPath).resolve()
        objBaseDirectory = objAbsPath.parent

        print(f"Processing {objAbsPath}")
        loadScene(objAbsPath, objBaseDirectory)

        pvsmPath = objAbsPath.parent / f"{objAbsPath.stem}-pv-state.pvsm"
        simple.SaveState(str(pvsmPath))
        simple.RenderAllViews()
